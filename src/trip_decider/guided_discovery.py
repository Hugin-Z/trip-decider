...
#: 候选比较阶段每个域的采集超时。
#:
#: railway 从 15 秒提到 40 秒：**实测真实 12306 查询要 15-30 秒**
#: （`scripts/smoke_live.py` 的记录），15 秒的预算让绝大多数候选在准入门槛前
#: 就超时。超时不是「到不了」——退回区会如实区分这两者，但一个全是超时的
#: 退回区对用户毫无价值，它只说明我们没给够时间。
#:
#: 代价是最坏情况下候选比较更慢。候选是并行采的，因此墙钟时间接近单个域的
#: 超时上限，不是三个相加。**标可调**：真实使用中若发现常态远低于此，回调。
_DEFAULT_TIMEOUTS = {
    "railway": 40.0,
    "map": 15.0,
    "web": 10.0,
