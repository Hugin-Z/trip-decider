...
**地名字面量集合**由测试显式维护，P1 种子清单来自两处实测：`src/trip_decider/destination_catalog.json` 中 28 个 `name` 字段的取值，以及 `PRODUCT.md:22,50-56` 自述的已验证链路涉及的地名。该清单是本不变式的一部分，新增地名后必须同步扩充——清单不完备会导致漏判，但不会导致误判。

### 当前状态：不成立

`src/trip_decider/destination_catalog.json` 硬编码 28 个中国目的地，属于 §3.1 拆分后仍归违规的一类（城市专属配置，非语言配置）。
