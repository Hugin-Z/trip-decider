> # ⛔ 本文已作废（2026-08-02）
> 
> 取代者：`p4b-baseline-flip-preview-v2.md`。
> 
> **作废原因**：§0 的 token 表量的是 `invariant_support.controlled_railway()`，
> 而表征场景实际用的是 `characterization_support.evidence()`——两个不同的夹具，
> 采集时刻差一天。据此推出的「7 场景各 +2 blocker」在真实夹具下不可达。
> 
> **正文一字不改。** 基准出错、被核对器的负向验证抓住、走三分法第三类处置，
> 这个过程本身是方法论的一部分，要可追溯。归因记录见 v2 §0。
